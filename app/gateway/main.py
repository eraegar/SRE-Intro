...
# Bulkhead (Lab 11 bonus) - bounds concurrent calls to payments.
...
async def call_with_retry(func, target: str, max_retries: int = RETRY_MAX):
    ...
    for attempt in range(max_retries):
        try:
            ...
            ) or (status is not None and (500 <= status <= 599 or status in (408, 429)))
...
class BulkheadFullError(Exception):
    """Raised when a downstream concurrency pool cannot accept a call."""
...
async def pay_reservation(reservation_id: str):
    ...
    try:
        ...
        log.error("payment protection rejected request: %s", exc)
        raise HTTPException(503, f"Payment service temporarily unavailable: {exc}")
    except httpx.TimeoutException:
